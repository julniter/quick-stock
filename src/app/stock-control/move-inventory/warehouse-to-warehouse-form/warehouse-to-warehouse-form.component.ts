...
export class WarehouseToWarehouseFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.pageMode === PageMode.New && this.router.url !== '/stock-control/move-inventory/warehouse-to-warehouse/new') {
      this.back();
    } else {
  ...
  onSubmit() {
    ...
    const origin = this.$db.moveInventory(MoveInventorySnapshotType.WarehouseToWarehouse).ref.doc();
    ...
    if (this.products.controls.length > 0 && inventoryForm.warehouseId !== inventoryForm.destinationWarehouseId) {
      ...
      const moveInventory = {
        ...
        type: MoveInventorySnapshotType.WarehouseToWarehouse,
        id: origin.id,
        createdAt: timeStamp,
        isActive: true,
        isDeleted: false
      ...
      this.$db.moveInventory(MoveInventorySnapshotType.WarehouseToWarehouse)
  ...
  loadLatestSnapshot() {
    const selectedWarehouse = this.warehouseItems.find(o => o.id === this.warehouseToWarehouseForm.getRawValue().warehouseId);
